...
def Make_M(V,Tau,Sig,UseMag):
    ...
    if alt == 0:
        ...
        funct = (V ** 2 + Sig**2) * (1- np.exp(-(delta_t_list)/Tau))
        M = np.diag(sigma_sq+Sig**2) + (V ** 2) * np.exp(-1.0 * (time_array)/Tau)
...
def lnlike(theta, time, delta_f, sigma_sq):
    if alt == 0:
        logV, logTau, logSig = theta
        inv_sigma2 = sigma_sq ** -1
        M,delta_f,sigma_sq = Make_M(10**logV,10**logTau,10**logSig,UseMag=False)
    elif alt == 1:
        logV, alpha = theta
        inv_sigma2 = sigma_sq ** -1
...
def lnprior(theta):
    if alt == 0:
        logV, logTau, logSig = theta
        if -3 < logV < 2 and 0 < logTau < 4 and -3 < logSig < 2:
            return 0.0
    elif alt == 1:
        logV, alpha = theta
...
def eq(p):
    A,B,C = p
    f1 = A * X[0]**2 + B*X[0] + C - Y[0]
    f2 = A * X[1]**2 + B*X[1] + C - Y[1]
    f3 = A * X[2]**2 + B*X[2] + C - Y[2]
    return(f1,f2,f3)
...
for alt in range(1):
    ...
    for row in range(1):
        ...
        if len(flux) < 20:
            ...
            DrawFig()
            M,delta_f,sigma_sq = Make_M(V,Tau,Sig,UseMag)
            #print(M)
            sign, value = np.linalg.slogdet(M)
            deter = sign * np.exp(value.item())
            ...
            if alt == 0:
                result = op.minimize(nll, [np.log10(V), np.log10(Tau),np.log10(Sig)],args=(time,delta_f,sigma_sq))
            elif alt == 1:
            ...
            logprobs = []
            logvals = []
            V_mcmc, Tau_mcmc, Sig_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]),
                                   zip(*np.percentile(samples, [16, 50, 84],
                                                      axis=0)))
        
        
            fig = corner.corner(samples, labels=[r"log$_{10}V$", r"log$_{10}\tau$",r"log$_{10}$Sig"],

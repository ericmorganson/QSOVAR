...
def get_vals(args, ROW):
    ...
    for color in "griz":
        ...
        if limit < 3:
            ...
            return (str(ROW) + " not enough "+ color + " obs")
...
def sausageplot_step2(Vari, time, delta_f, Tau, dt, sigma_sq, dMu_dict,
                      scale_dict, color_sort_ones, mu, ROW, fig):
    err_top = []
    err_bot = []
    Logpr = []
    Vari = 10 ** Vari
...
def perform_emcee_single(time, flux, err_2, dMu, scale, ROW, mu, color):
    ...
    if PLOTTING:
        sausageplot_single(max_theta[0], time, flux, max_theta[1], 5, err_2, dMu, scale, mu,  ROW, fig, color)
...
def perform_emcee_step2(time, flux, err_2, dMu_dict, scale_dict,
    ...
    if PLOTTING:
        sausageplot_step2(max_theta[0], time, flux, max_theta[1], 5, err_2, dMu_dict, scale_dict, color_sort_ones, mu,  ROW, fig)
        if var_count > 2:
    ...
    # WRITE THE FOUND MAX THETA VALUES TO FILE
    fit_str = str(sys.argv[1].split("/")[-1].split("_")[0])
    print("printing "+ str(ROW) + " to file!")
    filename = FILEPATH + str(ROW) +"_"+fit_str+"_"+ sys.argv[4] + "_all_band_linear_"+ str(sys.argv[5]) + '.txt'
...
def lin_color_sort(time, color_sort_dict, color_sort, flux):
...
def main_run(ROW_it):
    for ROW in [ROW_it]:
        ...
        if SKIPCOMPROW and os.path.exists(filename_all):
            ...
            return (str(ROW) + " already analyzed!")
        ...
        g_flux, i_flux, z_flux = lin_color_sort(time, color_sort_dict, color_sort, flux)
        ...
        if var_count < 1:
            plt.close("all")
            print("Not variable in " + str(ROW) +", closing plots & going to next row")
            return (str(ROW) + " not var!")
        ...
        if len(spread) > 1:
            ...
            print("Too spread in bands: "+ str(spread)+str(ROW))
            return (str(ROW) + " too spread!")
        if PLOTTING:
        ...
        try:
            ...
            return (str(ROW) + " worked!")

        except np.linalg.linalg.LinAlgError as err:
            print("Linear Algebra Error")
            return (str(ROW)+" Lin Alg Err")
...
if __name__ == "__main__":
    ...
    res = pool.map(main_run, range(int(sys.argv[2]), int(sys.argv[3])), chunksize=1)
    print(res)

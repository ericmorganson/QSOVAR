...
def perform_emcee_step2(time, flux, sigma_sq, color_sort_ones, dmu_scales, ROW, mu):
        ...
        fig1.savefig("figure/"+str(ROW)+sys.argv[4]+"_all_band_"+"triangle_step2"+str(sys.argv[5])+ ".pdf")
        ...
        #WRITE THE FOUND MAX THETA VALUES TO FILE
        filename ='scratch_new/'+ str(ROW) + sys.argv[4]+"_all_band_step2"+'object_dMu_' + str(sys.argv[5]) +'.txt'
        with open(filename, 'w+') as fout:
            fout.write('Object: ' + str(ROW)+ ' ' + 'Tau: ' + str(max_theta[1])+' ' + 'V: '+ str(max_theta[0]) + '\n')
...
for ROW in range(int(sys.argv[2]),int(sys.argv[3])):
    ...
    try:
        ...
        scale_dict = {"g":max_theta[6], "r":1, "i":max_theta[7], "z":max_theta[8]}

...
def lognorm(state, flux, flux_err_sq):
        ...
        fmean, V_sq = state
        #return np.log(np.exp(-((flux-fmean)**2)/(2*(V_sq + flux_err_sq)))/np.sqrt(2*np.pi*(V_sq + flux_err_sq))) #good & stable
        return -(flux**2/(2*(V_sq + flux_err_sq))) + (flux*fmean/(V_sq + flux_err_sq)) - (fmean**2/(2*(V_sq + flux_err_sq))) -0.5 *np.log(2*np.pi*(V_sq + flux_err_sq)) # assume all values are real; stable
...
def preform_emcee(time,delta_f,sigma_sq,ROW):
        ...
        plt.savefig('/home/sam/Documents/Morganson_research/QSOVAR/'+ str(ROW) + 'logprob_curve3d' + '.pdf')
        plt.show()
